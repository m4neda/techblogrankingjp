...
def main():
    rankings = []
    df_blog_url = pd.read_csv("techbloglist.csv", usecols=["company_name", "url"])
    dir_csv = "csv/"
    for filename in listdir(dir_csv):
        company_name = path.splitext(filename)[0]
        df = pd.read_csv(
            dir_csv + filename, parse_dates=["published_at"], index_col="published_at"
        )
        blog_url = df_blog_url.loc[
            df_blog_url["company_name"] == company_name
        ].url.item()
    ...
    df = pd.DataFrame(
        rankings,
        columns=["company_name", "article_count", "hatebu_count", "score", "url"],
    )
    df.insert(
        0, "rank", df["score"].rank(ascending=False, method="min", na_option="bottom")
    )
